...
-- Uses Telescope for the picker interface

local telescope_available, telescope = pcall(require, "telescope.builtin")
local pickers_available, pickers = pcall(require, "telescope.pickers")
local finders_available, finders = pcall(require, "telescope.finders")
local conf_available, conf = pcall(require, "telescope.config")
local actions_available, actions = pcall(require, "telescope.actions")
local action_state_available, action_state = pcall(require, "telescope.actions.state")
...
local function dbg(tag, payload)
  if DEBUG then
    vim.notify(("%s: %s"):format(tag, vim.inspect(payload)), vim.log.levels.DEBUG)
  end
end
...
local function apply(chat, new_msgs)
  ...
  vim.schedule(function()
    ...
    if line_count > 0 and vim.api.nvim_buf_is_valid(chat.bufnr) then
      ...
      -- Silently complete the operation
  end)
end
...
local function remove_chat_messages(chat)
  ...
  for i, msg in ipairs(messages) do
    if msg.role == "user" then
      items[#items + 1] = {
        msg_idx = i,
        text    = ("%s:%s"):format(msg.id or "no-id", msg.cycle or ""),
  ...
  if not (telescope_available and pickers_available and finders_available and 
          conf_available and actions_available and action_state_available) then
    vim.notify("Telescope is required for chat message removal picker", vim.log.levels.ERROR)
    return
  end
